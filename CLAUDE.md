...
- Mobbin is the design source of truth: before designing/redesigning any page or section, pull 2–3 real examples via the Mobbin MCP (`search_sections` for marketing-site sections — hero, pricing, footer; `search_screens` for whole pages; tool names may carry a server prefix — load via ToolSearch if deferred) with `platform: "web"`, naming a top app to filter. Study the returned screenshot images — layout, hierarchy, spacing — not the metadata. Rule: adapt the pattern, never copy the look — colors/type come from the design-system + cbip tokens. When presenting design directions to Isaac, cite each referenced screen as a markdown link to its `mobbin_url`.
